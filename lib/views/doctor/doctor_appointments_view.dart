...
class DoctorAppointmentsScreen extends StatelessWidget {
      ...
      child: Column(
        ...
        children: [
          Icon(
            ...
            size: 64,
            color: slate600.withOpacity(0.3),
          ),
          ...
          child: Column(
            children: [
              ...
              if (requests.isNotEmpty) ...[
                const SizedBox(height: 12),
                ...requests.map(
                      (requestDoc) => _buildRequestActions(
                    context,
                    clinic,
                    requestDoc,
                    viewModel,
            ...
            if (status == 'accepted' && vitalsEntered) ...[
              Expanded(
                child: _actionBtn(
                  ...
                  onTap: () => _showVitalsDialog(context, data['vitals']),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
  ...
  Future<void> _openOnlineVideoCall(
      BuildContext context,
      DoctorAppointmentsViewModel viewModel, {
    ...
    final shouldEndAppointment = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
    ...
    if (shouldEndAppointment == true) {
      ...
      try {
        ...
        if (context.mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Error ending appointment: $e"),
  ...
  void _showEndAppointmentOptions(
      BuildContext context,
      DoctorAppointmentsViewModel viewModel,
      String clinicId,
  ...
  void _showManualPrescriptionDialog(
      BuildContext context,
      DoctorAppointmentsViewModel viewModel,
      String clinicId,
      String appointmentId,
  ...
  Future<void> _handlePrescriptionUpload(
      BuildContext context,
      DoctorAppointmentsViewModel viewModel,
      String clinicId,
      String appointmentId,
  ...
  Future<void> _handleAccept(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          smsSent ? "Accepted & SMS sent." : "Accepted, SMS failed.",
        ),
      ),
    );
  }
    ...
    return FutureBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data!.exists) {
          refNumber =
              (snapshot.data!.data() as Map<String, dynamic>)['referenceNumber'] ??
                  'N/A';
        }
          ...
          child: _actionBtn(
            ...
            onTap: () async {
              ...
              if (user != null) {
                ...
                if (docSnap.exists) {
                  fetchedDocName =
                      (docSnap.data() as Map<String, dynamic>)['name'] ??
                          "Doctor";
                }
              }
